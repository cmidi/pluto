...
GetOptions ( "host=s"     => \$sbcInfo{'host'},
	     ...
	     "ftpip=s"    => \$ftpInfo{'IP'},
             ...
             "interval=s" => \$hpingInfo{'interval'},
             "count=i"    => \$hpingInfo{'count'},
             "attack=s"   => \$hpingInfo{'attack'},
             "intf=s"     => \$hpingInfo{'intf'},
...
my $tcpStats = "show datapath etc-stats ppms transport tcp-stats";
my $tlsStats = "show datapath etc-stats ppms tls tls-stats";
my @tcpStats = "";
my @tlsStats = "";
...
#
# Create client/server/syn flood instance instances of Transport Test Tool this requires  
...
for($i=0;$i<$instances;$i++)
{
    ...
    if($i == ($instances-1))
    {
          $startTTServer = "sudo hping3 $hpingInfo{'attack'} -p $transportTTargs{'prp'} $transportTTargs{'pra'} -i $hpingInfo{'interval'}".
                           " --rand-source -I $hpingInfo{'intf'} -c $hpingInfo{'count'}";
    }
    else
    {
...
foreach my $server (@servers)
{
    ...
    {
        @resultTT = $devSession[$i]->cmd(String=>"$server",Timeout=>500,Prompt => "/Test Complete|statistic/"); 
        foreach(@resultTT)
        {
...
my $synCacheTimeout = 0;
my $synCacheAdds     = 0;
my $synCacheCompleted = 0;
if(($transportTTargs{'ctype'} eq 'TLS') || ($transportTTargs{'stype'} eq 'TLS'))
{
    @tlsStats = $session->cmd(String=>"$tlsStats",Timeout=>30,Prompt=>"/#/");
}
else
{
    @tcpStats = $session->cmd(String=>"$tcpStats",Timeout=>30,Prompt=>"/#/");
    foreach(@tcpStats)
    {
         if(/TCP Syncache Timeout\s*(\d*)/)
         {
             $synCacheTimeout = $1;
         }
         if(/Syncache Adds:\s*(\d*)/)
         {
             $synCacheAdds = $1;
         }
         if(/Syncache Completed:\s*(\d*)/)
         {
             $synCacheCompleted = $1;
         }
...
log_msg("Total SYN Cache Adds            :$synCacheAdds");
log_msg("Total SYN Cache Completed       :$synCacheCompleted");
log_msg("Total SYN Cache Timeouts        :$synCacheTimeout");
print ("\n");
log_msg("--------------!!!END STATS!!!----------------");
print ("\n");
print ("\n");
